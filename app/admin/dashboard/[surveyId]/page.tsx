...
interface AnalyticsData {
  ...
  questionAnalytics: {
    ...
    chartData: { answer: string; count: number; fullAnswer: string }[];
  }[];
}
...
export default function SurveyResultsPage() {
  ...
  const [currentView, setCurrentView] = useState<"responses" | "analytics">(
    "analytics"
  );
  ...
  const generateAnalytics = (
    ...
    const responsesByDay = Array.from({ length: 7 }, (_, i) => {
      ...
      const dayResponses = responses.filter(
        (r) => format(new Date(r.submitted_at), "MMM dd") === dateStr
      ).length;
    ...
    const questionAnalytics = questions.map((question) => {
      ...
      let chartData: { answer: string; count: number; fullAnswer: string }[] =
        [];
      ...
      if (
        ...
        questionResponses.forEach((answer) => {
          const choice = answer.answer_choice
            ? JSON.parse(answer.answer_choice as string)
            : answer.answer_text;
              ...
              {analytics.questionAnalytics.map((qa, index) => (
                ...
                >
                  ...
                  {qa.chartData.length > 0 && (
                    <div className="h-64">
                      <ResponsiveContainer width="100%" height="100%">
                        {qa.questionType === "multiple_choice" ||
                        qa.questionType === "likert_scale" ? (
                          <PieChart>
                            <Pie
                              data={qa.chartData}
                              cx="50%"
                          ...
                          <div className="p-4 bg-gray-50 rounded-lg">
                            <p className="text-gray-600">
                              Text responses cannot be visualized in charts.
                            </p>
                  ...
                  <div className="overflow-x-auto">
                              ...
                              if (answer) {
                                if (answer.answer_text) {
                                  ...
                                  const choice = JSON.parse(
                                    answer.answer_choice as string
                                  );
                                  displayValue = Array.isArray(choice)
                                    ? choice.join(", ")
                                    : choice;
                                }
                              }
                    ...
                    {responses.length > 10 && (
                      <div className="mt-4 text-center">
                        <p className="text-gray-600">
                          Showing 10 of {responses.length} responses. Export CSV
                          to see all data.
                        </p>
                      </div>
                    )}
